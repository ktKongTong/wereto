...
import { bulkUpsert, upsertOne } from "../db/utils/d1-bulk-writer.ts";
...
export async function stageSnapshot(db: AppDb, runId: number, targetTable: SnapshotTarget, entityKey: string, payload: unknown) {
  ...
  switch (targetTable) {
    case "books":
      return upsertOne(db, syncSnapshotBooks, [syncSnapshotBooks.runId, syncSnapshotBooks.wereadBookId], {
    ...
    case "albums":
      return upsertOne(db, syncSnapshotAlbums, [syncSnapshotAlbums.runId, syncSnapshotAlbums.wereadAlbumId], {
    ...
    case "shelf_items":
      return upsertOne(db, syncSnapshotShelfItems, [syncSnapshotShelfItems.runId, syncSnapshotShelfItems.entityKey], {
    ...
    case "notebook_books":
      return upsertOne(db, syncSnapshotNotebookBooks, [syncSnapshotNotebookBooks.runId, syncSnapshotNotebookBooks.wereadBookId], {
    ...
    case "book_info":
      return upsertOne(db, syncSnapshotBookInfo, [syncSnapshotBookInfo.runId, syncSnapshotBookInfo.wereadBookId], {
    ...
    case "book_progress":
      return upsertOne(db, syncSnapshotBookProgress, [syncSnapshotBookProgress.runId, syncSnapshotBookProgress.wereadBookId], {
    ...
    case "highlights":
      return upsertOne(db, syncSnapshotHighlights, [syncSnapshotHighlights.runId, syncSnapshotHighlights.wereadBookmarkId], {
    ...
    case "reviews":
      return upsertOne(db, syncSnapshotReviews, [syncSnapshotReviews.runId, syncSnapshotReviews.wereadReviewId], {
    ...
    case "reading_periods":
      return upsertOne(db, syncSnapshotReadingPeriods, [syncSnapshotReadingPeriods.runId, syncSnapshotReadingPeriods.periodType, syncSnapshotReadingPeriods.periodStart], {
    ...
    case "reading_period_books":
      return upsertOne(db, syncSnapshotReadingPeriodBooks, [syncSnapshotReadingPeriodBooks.runId, syncSnapshotReadingPeriodBooks.periodKey, syncSnapshotReadingPeriodBooks.rank], {
    ...
    case "reading_years":
      return upsertOne(db, syncSnapshotReadingYears, [syncSnapshotReadingYears.runId, syncSnapshotReadingYears.year], {
    ...
    case "reading_top_books":
      return upsertOne(db, syncSnapshotReadingTopBooks, [syncSnapshotReadingTopBooks.runId, syncSnapshotReadingTopBooks.year, syncSnapshotReadingTopBooks.rank], {
    ...
    case "reading_days":
      return upsertOne(db, syncSnapshotReadingDays, [syncSnapshotReadingDays.runId, syncSnapshotReadingDays.year, syncSnapshotReadingDays.day], {
        runId, createdAt, ...pick(item, ["year", "day", "readSeconds", "source"]),
      }, ["read_seconds", "source", "created_at"]);
    case "sync_cursors":
      return upsertOne(db, syncSnapshotCursors, [syncSnapshotCursors.runId, syncSnapshotCursors.key], {
...
export async function stageReadingDaySnapshots(
  ...
  await bulkUpsert(db, syncSnapshotReadingDays, [syncSnapshotReadingDays.runId, syncSnapshotReadingDays.year, syncSnapshotReadingDays.day], values, [
    "read_seconds",
    "source",
    "created_at",
  ]);
...
export async function stageHighlightSnapshots(db: AppDb, runId: number, rows: Record<string, unknown>[]) {
  ...
  await bulkUpsert(db, syncSnapshotHighlights, [syncSnapshotHighlights.runId, syncSnapshotHighlights.wereadBookmarkId], values, [
    "weread_book_id",
    "chapter_uid",
    "chapter_title",
    "range",
...
export async function stageReviewSnapshots(db: AppDb, runId: number, rows: Record<string, unknown>[]) {
  ...
  await bulkUpsert(db, syncSnapshotReviews, [syncSnapshotReviews.runId, syncSnapshotReviews.wereadReviewId], values, [
    "weread_book_id",
    "chapter_uid",
    "chapter_name",
    "range",
